...
public class Sorting {
    ...
    /*
     * Create an experiement to run through large Array Sizes
     * Merge sort should technically be faster than insertion sort
     * 
     */

    private static final int ARRAYSIZE = 2;
      private static final int someNUMBER = 100000; // Interchangable random number
      private static final int MULTIPLIER = 2;
      private static final int MIN = 0;
      private static final int MAX = 100;

    static void experiment () {
        long start; // Long is similar to int and double but longer
        long how_long; 
        int arraySize = ARRAYSIZE;

      while(arraySize < someNUMBER) {
        int []randomArray = randomArray(arraySize, MIN, MAX);// randomly populate array of length array size
         start = System.nanoTime(); // Start the test taking in compiler time

        insertionSort(randomArray); // First the insertion test
        how_long = System.nanoTime() - start; // Take the time - its start time
            System.out.println("Insertion Sort " + arraySize + ", " + how_long); // Print out with x array size how long it took

    
            start = System.nanoTime(); // Restart test

            mergeSort(randomArray); // Test merge sort

           how_long = System.nanoTime() - start;
            System.out.println("Merge Sort " + arraySize + ", " + how_long); // Same principles
            System.out.println(); // Just to make the data more organized
        arraySize = arraySize * MULTIPLIER; // Just to sort out the magic values
